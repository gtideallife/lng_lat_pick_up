  ...
  render() {
    ...
    const search = () => {
      if (this.state.searchKey) {
        const url = `http://restapi.amap.com/v3/place/text?keywords=${
      ...
      <div className="App">
        ...
        <div className="operation">
          {this.state.polygonData.length === 0 ? (
            ...
            <Fragment>
              <button onClick={editPolygon}>{this.state.openEdit ? '完成编辑' : '编辑区域'}</button>
        ...
        {this.state.searchPoi && (
          <div className="showSearch">
            <div>搜索结果</div>
            <div>坐标：{this.state.searchPoi.location}</div>
            <div>描述：{this.state.searchPoi.address}</div>
            <div>类型：{this.state.searchPoi.type}</div>
          </div>
        )}
        {this.state.polygonData.length !== 0 && (
          <div className="showData">{JSON.stringify(this.state.polygonData)}</div>
        )}
      </div>
    );
  }
